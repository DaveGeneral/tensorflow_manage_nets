...
conv4_weights = tf.get_variable("Weights_4", shape=[PATCH_SIZE, PATCH_SIZE, DEPTH_3, DEPTH_4])
...
cl_l3_biases = tf.Variable(tf.constant(0.05, shape=[DEPTH_4]), name='Classifer_Biases_1')
...
cl_out_weights = tf.get_variable("Classifer_Weights_3", shape=[DEPTH_4, CL_NUM_LABELS])
cl_out_biases = tf.Variable(tf.constant(0.05, shape=[CL_NUM_LABELS]), name='Classifer_Biases_3')
...
def convolution_model(data):
    with tf.variable_scope('Layer_1', reuse=True) as scope:
        con = tf.nn.conv2d(data, conv1_weights, [1, 1, 1, 1], 'VALID', name='C1')
        hid = tf.nn.relu(con + conv1_biases)
        activation_summary(hid)

    pol = tf.nn.max_pool(hid, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME', name='Pool_1')
    lrn = tf.nn.local_response_normalization(pol, name="Normalize_1")

    with tf.variable_scope('Layer_2') as scope:
        con = tf.nn.conv2d(lrn, conv2_weights, [1, 1, 1, 1], padding='VALID', name='C3')
        hid = tf.nn.relu(con + conv2_biases)
        activation_summary(hid)

    pol = tf.nn.max_pool(hid, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME', name='Pool_2')
    lrn = tf.nn.local_response_normalization(pol, name="Normalize_2")

    with tf.variable_scope('Layer_3') as scope:
        con = tf.nn.conv2d(lrn, conv3_weights, [1, 1, 1, 1], padding='VALID', name='C5')

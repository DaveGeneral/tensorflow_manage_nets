...
def train_classification(train_data, train_labels,
    ...
    learning_rate = tf.train.exponential_decay(LEARN_RATE, global_step*BATCH_SIZE, train_size, DECAY_RATE, staircase=STAIRCASE)
    ...
    with tf.Session(config=c) as sess:
        ...
        train_writer = tf.train.summary.FileWriter(TENSORBOARD_SUMMARIES_DIR + '/train', sess.graph)
        valid_writer = tf.train.summary.FileWriter(TENSORBOARD_SUMMARIES_DIR + '/validation')
        ...
        for step in range(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
            ...
            if step % 1000 == 0:
                ...
                train_summary, _, l, lr, train_acc = sess.run([merged, optimizer, loss, learning_rate, accuracy], feed_dict=feed_dict, options=run_options, run_metadata=run_metadata)

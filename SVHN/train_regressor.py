...
def train_regressor(train_data, train_labels, valid_data, valid_labels, test_data, test_labels, train_size, saved_weights_path):
    ...
    with tf.name_scope('input'):
        images_placeholder = tf.placeholder(tf.float32, shape=(BATCH_SIZE, IMG_HEIGHT, IMG_WIDTH, NUM_CHANNELS))

    with tf.name_scope('image'):
        tf.summary.image('input', images_placeholder, 10)

    labels_placeholder = tf.placeholder(tf.int32, shape=(BATCH_SIZE, LABELS_LEN))
    ...
    prediction = tf.stack([tf.nn.softmax(regression_head(images_placeholder)[0]),
                          ...
                          tf.nn.softmax(regression_head(images_placeholder)[4])], axis=0)
